...
| 21 | `vulkan: drain the queue before vkResetCommandPool in ggml_vk_command_pool_cleanup` | ours — fixes issue #215 moss-transcribe segfault; async `graph_compute` defers sync, so `command_pool_cleanup` can `vkResetCommandPool` while buffers are still pending (faults on RADV/NVIDIA, needs ≥10 accumulated buffers — the "multiple slices" trigger). ~11 LOC `waitIdle` guard at the choke point; `.patch` included | **draft — patch applied locally, awaiting native RADV/NVIDIA confirmation** (MoltenVK can't reproduce; set `CRISPASR_MOSS_TRANSCRIBE_VULKAN_ASYNC=1` to exercise the async path); then file at ggml-org/llama.cpp (vulkan) |
